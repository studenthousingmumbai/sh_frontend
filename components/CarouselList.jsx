...
export default function CarouselSize({
  ...
  const intervalTime = 5000; // (5000ms = 5 seconds)

  useEffect(() => {
    if (autoScroll && api) {
      const interval = setInterval(() => {
        if (!isHoveredRef.current) {
          api.scrollTo((current + 1) % items.length);
        }
      }, intervalTime);

      return () => clearInterval(interval);
    }
  }, [api, current, autoScroll, intervalTime, items.length]);
            ...
            <CarouselItem
              ...
              className={`${
                componentName === "student-testimonials"
                  ...
                  : "basis-full md:basis-1/2 lg:basis-1/3 2xl:basis-1/4"
              }`}
            >
              {renderItem(item)}
            </CarouselItem>
